...
window.CALORIYA = {
  ...
  // === Аналитика ==============================================================
  // Тот же счётчик, что на my.calorybot.ru и pay.calorybot.ru, и это условие
  // работы воронки, а не экономия. Cookie Метрики живут на домене второго
  // уровня, поэтому у calorybot.ru, my. и pay. один ClientID; переходы между
  // ними считаются внутренними, потому что все три перечислены в адресах
  // счётчика (calorybot.ru — основной, остальные два — дополнительные).
  //
  // Два счётчика вместо одного сломали бы кампанию, ведущую на лендинг: оплата
  // происходит на my./pay., значит цель trial_paid жила бы в другом счётчике, и
  // оптимизироваться на неё было бы нечем.
  //
  // ⚠️ Номер лежит в трёх репозиториях — здесь, @calorybot
  // frontend/assets/metrika.js и @billing src/pay/index.html. Общей сборки нет,
  // при смене править все три. Публичность номера не проблема: он виден в
  // исходнике страницы у каждого посетителя.
  //
  // Политика конфиденциальности (§9.2) уже объявляет Метрику, включая Вебвизор.
  // null отключает счётчик целиком — это режим для локальной разработки.
  METRIKA_ID: 112507894,
